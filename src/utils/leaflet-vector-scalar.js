...
L.ScalarTileLayer = (L.GridLayer ? L.GridLayer : L.Class).extend({
  options: {
    displayValues: true,
    displayOptions: {
      velocityType: "Scalar",
  ...
  _setupColorScale: function () {
    ...
    this._colorScale = µ.segmentedColorScale(COLORSCALE);
    this._overlayAlpha = OVERLAY_ALPHA;
    this._transparentBlack = TRANSPARENT_BLACK;
  },
  ...
  setOpacity: function setOpacity(opacity) {
    ...
    if (this._map) {
      this.getContainer().style.opacity = opacity;
    }
  },
  ...
  createTile: function (coords) {
    if (!µ.isValue(this._windData) || !µ.isValue(this._header)) {
      // console.warn('数据或头信息为空，返回空瓦片');
      var emptyTile = document.createElement("canvas");
      emptyTile.width = emptyTile.height = this.options.tileSize;
      return emptyTile;
    }
    ...
    for (var j = 0; j < this.options.tileSize; j++) {
      for (var i = 0; i < this.options.tileSize; i++) {
        ...
        pixelCount++;
        if (µ.isValue(value) && !isNaN(value)) {
          // 使用 µ.isValue
          validValueCount++;
        }
      }
  ...
  _tilePixelToLatLng: function (i, j, coords) {
  ...
  _latLngToWindGridIndex: function (lat, lon, header) {
    var lo1 = header.lo1,
      la1 = header.la1,
      lo2 = header.lo2,
      la2 = header.la2;
    ...
    var Δλ = (lo2 - lo1) / (nx - 1);
    var fi = µ.floorMod(lon - lo1, 360) / Δλ; // 使用 µ.floorMod
    var fj = ((la1 - lat) / (la1 - la2)) * (ny - 1);

    return { fi: fi, fj: fj };
  },

  _bilinearInterpolate: function (arr, nx, ny, fi, fj, isContinuous) {
    ...
    var v00 = getVal(i0, j0);
    var v10 = getVal(i1, j0);
    var v01 = getVal(i0, j1);
    var v11 = getVal(i1, j1);
  ...
  _getColor: function (value) {
    if (!µ.isValue(value)) {
      return this._transparentBlack;
    }
    var color = this._colorScale(value, this._overlayAlpha);
...
L.scalarTileLayer = function (options) {
  return new L.ScalarTileLayer(options);
};
...
var Scalar = function Scalar(params) {
  ...
  var bilinearInterpolateScalar = function bilinearInterpolateScalar(
    x,
    y,
    g00,
    g10,
...
L.VectorArrowLayer = L.CanvasLayer.extend({
  options: {
    color: "#ffffff",
    arrowSize: 20,
    gridSize: 50,
...
L.vectorArrowLayer = function (options) {
  return new L.VectorArrowLayer(options);
};

L.VectorBarbLayer = L.VectorArrowLayer.extend({
  options: {
    color: "#333333",
    barbSize: 30,
    gridSize: 60, // 风杆通常需要更大间距以免重叠
  },
  _drawArrow: function (ctx, x, y, u, v) {
    // Reuse grid logic from VectorArrowLayer, but override draw method
    this._drawBarb(ctx, x, y, u, v);
  },
  _drawBarb: function (ctx, x, y, u, v) {
    var speedMS = Math.sqrt(u * u + v * v);
    if (speedMS < 0.5) return; // calm

    // Convert m/s to knots (1 m/s = 1.94384 knots)
    var speedKnots = speedMS * 1.94384;
    // Round to nearest 5
    var speed = Math.round(speedKnots / 5) * 5;

    // Angle: Wind comes FROM.
    // Vector (u, v) points WHERE wind goes.
    // Screen Y is down (-v is North).
    // Vector angle on screen: atan2(-v, u).
    // Barb shaft points INTO the wind (opposite to vector).
    var angle = Math.atan2(-v, u) + Math.PI;

    var len = this.options.barbSize;
    ...
    // Draw shaft: from (0,0) station to (len, 0)
    ctx.beginPath();
    ctx.moveTo(0, 0);
    ctx.lineTo(len, 0);
    ctx.stroke();

    // Draw station circle (optional, often just the start of shaft)
    // ctx.beginPath();
    // ctx.arc(0, 0, 2, 0, 2 * Math.PI);
    // ctx.fill();

    // Draw barbs starting from the tail (len)
    var pos = len;
    var barbLen = 10;
    var slant = 3; // Slant towards station

    // Pennants (50 knots) - Triangle
    while (speed >= 50) {
      ctx.beginPath();
      ctx.moveTo(pos, 0);
      ctx.lineTo(pos - 5, -barbLen);
      ctx.lineTo(pos - 10, 0);
      ctx.fill(); // Filled triangle
      pos -= 10; // Skip space
      speed -= 50;
    }
    ...
    while (speed >= 10) {
      ctx.beginPath();
      ctx.moveTo(pos, 0);
      ctx.lineTo(pos - slant, -barbLen);
      ctx.stroke();
      pos -= 4; // Space between barbs
      speed -= 10;
    }
    ...
    if (speed >= 5) {
      ctx.beginPath();
      ctx.moveTo(pos, 0);
      ctx.lineTo(pos - slant / 2, -barbLen / 2);
      ctx.stroke();
    }
