<template>
  <div class="test-page">
    ...
    <div class="auto-refresh-switch">
      <span>自动刷新</span>
      <a-switch :checked="autoRefresh" @change="onAutoRefreshChange" />
    </div>
...
import MapStatus from "@/components/MapStatus/index.vue";
...
const initMap = () => {
  ...
  // 监听 zoom 变化
  map.value.on("zoomend", onZoomChange);
...
const onZoomChange = () => {
  ...
  const newShowLabel = zoom >= 10;
  // 通过 store 更新标签显示状态
  testPageStore.setShowLabel(newShowLabel);
}

const addPoints = () => {
  // 先把数据存入 Pinia store
  testPageStore.setPoints(points.value);
  ...
  points.value.forEach((point) => {
    ...
    if (iconEl) {
      ...
      const markerInstance = createApp(CircleMarker, {
        id: point.id,
        size: 24,
      ...
      // 挂载到临时容器
      markerInstance.mount(container)
      // 把容器添加到 marker 元素中
...
// 刷新数据（随机更新风力等级和风向）
const refreshData = () => {
  // 更新 Pinia store 中的数据，Vue 响应式会自动更新组件
  points.value.forEach((point) => {
    testPageStore.updatePoint({
      id: point.id,
      data: {
...
onUnmounted(() => {
  ...
  // 销毁所有 Vue 组件实例
  markerInstances.value.forEach((instance) => {
    instance.unmount()
  });
  if (map.value) {
