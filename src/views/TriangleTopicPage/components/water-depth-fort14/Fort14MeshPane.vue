<template>
  <div class="pane">
    <div class="tab-meta">
      <div class="tab-title">水深 fort.14 模块</div>
      <div class="tab-desc">
        这是从 `地图-三角图层/14原数据解析渲染` 抽出来的水深网格模块。
        数据链路是 `fort.14 -> mesh.json`，页面里用 Canvas + GridLayer 渲染，并保留点击查询水深的能力。
      </div>
    </div>
    ...
    <div class="memory-card">
      ...
      <div class="memory-grid">
        <div class="memory-item">
          ...
          <div class="memory-sub">约 21.67 MB，353,326 个三角形、189,136 个节点</div>
        </div>
        <div class="memory-item">
        ...
        <div class="memory-item">
          ...
          <div class="memory-sub">约 13.73 MB，字段为 numNodes / numElements / nodes / elements</div>
        </div>
      </div>
    ...
    <div class="preview-shell">
      ...
      <div class="panel status-panel">
        <div class="panel-title">水深 fort.14</div>
...
function depthToColor(depth) {
  const stops = [
    { val: -1.75, r: 0, g: 255, b: 255 },
    { val: 6309, r: 6, g: 3, b: 255 },
  ];

  if (depth <= stops[0].val) {
    return `rgb(${stops[0].r},${stops[0].g},${stops[0].b})`;
  }
  if (depth >= stops[1].val) {
    return `rgb(${stops[1].r},${stops[1].g},${stops[1].b})`;
  }

  const t = (depth - stops[0].val) / (stops[1].val - stops[0].val);
  const r = Math.round(stops[0].r + t * (stops[1].r - stops[0].r));
  const g = Math.round(stops[0].g + t * (stops[1].g - stops[0].g));
  const b = Math.round(stops[0].b + t * (stops[1].b - stops[0].b));
  return `rgb(${r},${g},${b})`;
}
...
const MeshTileLayer = L.GridLayer.extend({
  ...
  createTile(coords) {
    ...
    const { nodes, elements, numElements } = this._data;
...
export default {
  ...
  data() {
    return {
      ...
      legendLabels: ["6309.00", "5047.25", "3785.50", "2523.75", "1262.00", "-1.75"],
    };
  },
  computed: {
    ...
    async loadMesh() {
      try {
        const response = await fetch(
          "/data/triangle-topic/water-depth-fort14/mesh.json"
        );
