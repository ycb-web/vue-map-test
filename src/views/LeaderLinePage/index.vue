...
const onMapReady = (mapInstance: any) => {
  ...
  // 设置地图视图到广东区域
  map.value.setView([22.5, 113.5], 7)
...
// 面板位置变化
const onPanelMoved = (panelId: string, newPosition: { x: number; y: number }) => {
...
// 切换单个面板显隐
const onTogglePanel = (panelId: string, visible: boolean) => {
  const panel = panels.value.find((p) => p.id === panelId)
  if (panel) {
...
// 自动布局开关变化
const onAutoLayoutChange = (enabled: boolean) => {
...
const onScreenshot = async () => {
  ...
  // 临时隐藏控制面板
  const controlPanelEl = controlPanel.value?.$el
  if (!controlPanelEl) return

  const originalDisplay = controlPanelEl.style.display
  ...
  try {
    ...
    // 恢复控制面板显示
    controlPanelEl.style.display = originalDisplay
  }
}
</script>
